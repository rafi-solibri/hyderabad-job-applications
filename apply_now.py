...
def queue() -> list[dict]:
    ...
    for job in BATCH:
        ...
        blob_url = f"{job.get('url') or ''} {job.get('apply_url') or ''}".lower()
        if "salesforce.wd" in blob_url or "salesforce.com/" in blob_url:
            continue
        if re.search(
            r"devops|devsecops|site reliability|\bsre\b|sales engineer|"
            r"customer engineering|it automation|machine learning|\bmlops\b|"
            r"layout|embedded|pcie|firmware|rtl |asic |verification engineer|, verification\b|"
            ...
            r"\bjava\b|\bpython\b|spring boot|d365|dynamics 365|\bappian\b|drupal|"
            r"bangkok|gurugram based|relocation provided|"
            r"civil \(|uk water|linux kernel|device driver|"
            r"\baem\b|\bboomi\b|staff engineer,? electrical|electrical engineer|"
...
def _hosts_are_career_portal(hosts: list[str]) -> bool:
    blob = " ".join(hosts)
...
def is_aggregator_board(job: dict) -> bool:
    """True for Naukri / LinkedIn / Indeed / Cutshort / Foundit / Instahyre listing pages.

    Jobs discovered on Foundit/LinkedIn but pointing at Workday/Greenhouse/Oracle
    career URLs are company portals, not aggregator boards.
    """
    hosts = _job_hosts(job)
    if _hosts_are_career_portal(hosts):
        return False
    for host in hosts:
        if any(host == suffix or host.endswith("." + suffix) for suffix in AGGREGATOR_HOST_SUFFIXES):
            return True
    ats = (job.get("ats") or "").strip().lower()
    return ats in AGGREGATOR_ATS
...
def pick_best_per_company(jobs: list[dict], used: dict[str, set] | None = None) -> list[dict]:
    """Keep matching leftover jobs. Dedupe by company+title only — no per-company cap."""
