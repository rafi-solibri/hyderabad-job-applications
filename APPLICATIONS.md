# Hyderabad career-portal applications

Candidate: Mohammed Abdul Rafi Ahmed  
Resume: `data/resume/Rafi_Resume_Technical_Architect.docx`  
Current CTC: 52 LPA | Expected CTC: 65 LPA | Notice: Immediate  
Phone: +91 8790251698 | Email: rafi.success@gmail.com

**Areas:** Madhapur, Gachibowli, Financial District, HITEC City, Nanakramguda, Kondapur, Raidurg, Hyderabad, or Remote India.  
**Roles:** Technical / Solution / Software / Cloud / .NET Architect, Principal / Staff Engineer, Technical Lead, Engineering Manager, Senior Software / Backend / Full Stack / .NET.  
**Skip:** DevOps / SRE, TTEC, Keyloop, Redwood, Salesforce / ServiceNow / SAP / PEGA, Java-only or other-city office roles.  
**Cap:** 4 jobs per company. **Pace:** 50 applications / day, about 1000 in 20 days.

## Today (17 Aug 2026)

Submitted on Greenhouse / Lever / Amazon (confirmations):

- Crunchyroll, HighRadius, Inovalon, Postman, Zscaler (Greenhouse)
- Amazon: Principal SWE Shipping, Principal SWE DEX, Senior SDE AZA (2), Senior SDE Global Logistics
- You also applied Keyloop Principle Software Architect yourself

Blocked or skipped: TTEC and Keyloop (Lever verify error), Cprime (same error), Redwood (you asked to pause), DevOps titles, Highspot / Matillion (closed).

Smart fill is on for tomorrow: profile + learned answers, India + `8790251698`, essays from your Nemetschek / .NET / cloud background. Bad “Cursor” answers were removed.

## Today (18 Aug 2026)

Final list rebuilt in `DISCOVERY.md` from yesterday’s full title list, Hyderabad / Remote India, portals + job boards, 65 LPA (unknown CTC kept).

**Ready (under company cap):** Coinbase Engineering Manager (Remote India), Fivetran Staff SWE dbt core (Remote India).  
**Open but at 4/company:** Amazon DEX (tab open), Amazon Sr SDE Payroll, Twilio Staff L4.  
Broadcom Senior Software Architect submitted. HighRadius +2 Senior Solution Principal submitted earlier today.

1. Say you are ready in this chat.
2. I fill forms; you handle CAPTCHA only.
3. Target **50 submits**. New companies first.

Portals list **Hyderabad / Telangana**, not neighborhood names. Madhapur / Gachibowli / FD / HITEC jobs show up as Hyderabad. Remote India that matches your titles is included.

Microsoft, Google, Infosys, Deloitte, Wells Fargo, JPMC, and most Workday sites need a logged-in session. Those are next after public Greenhouse / Lever / Amazon.

## Answers used on every form

- Sponsorship: No | Authorized in India: Yes | Right-to-work: Indian Passport  
- Notice: Immediate | Start: Immediate / ASAP  
- Current CTC: 5200000 | Expected: 6500000 (65 LPA)  
- Company: Nemetschek | Title: Principal Analyst (Technical Architect)  
- Education: B.Tech IT, Acharya Nagarjuna University, 2006–2010  
- DOB: 16 Jan 1989 | Gender: Male  
- LinkedIn: https://linkedin.com/in/rafi-ahmed-mohammed-abdul-151644ba  
- Phone country: India (+91) | Phone: 8790251698

## Today (20 Aug 2026)

Cloud headed-Chrome apply with Simplify Copilot loaded. **Submitted 0**.

Google sign-in failed: saved form password is not the Google password. Requested `GOOGLE_PASSWORD` secret for rafi.success@gmail.com. LinkedIn email/password hit a security checkpoint/CAPTCHA.

Without a Google session, LinkedIn/Foundit/Instahyre were recorded as login-blocked. Coupa Lever and JPMC Oracle Cloud hit CAPTCHA. Schwab/Workday/SmartRecruiters Apply was clicked but privacy/sign-in gates were not completed.

Next run after GOOGLE_PASSWORD: Sign in with Google, then LinkedIn Easy Apply + Simplify Autofill.
